...
class ScilabPlotMagic(Magic):
    ...
    @option(
        '-a', '--antialiasing', action='store',
        help='To disable antialiasing set it to False'
    )
    ...
    def line_plot(self, *args, **kwargs):
        ...
        for key in ['resolution', 'format', 'size', 'width', 'height', 'antialiasing']:
...
def register_magics(kernel):
    kernel.register_magics(ScilabPlotMagic)
